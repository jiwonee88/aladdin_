...
?>
                    ...
                    <ul>
                        ...
                        <li>
						 ...
						 <li>
							...
							<input type="text" class="storeBuy myinfo_input" value="<?=$member[mb_bank]?$member[mb_bank].' '. $member[mb_bank_num].' '. $member[mb_bank_user]:'미등록'?>">
                        </li>
                         <li>
                            <span class="f_yellow">USDT 지갑 정보</span>
							<input type="text" class="storeAddr myinfo_input" value="<?=$member[mb_wallet_addr_u]?$member[mb_wallet_addr_u]:'미등록'?>">
                        </li>
                         <li>                          
                            <span class="f_yellowDesc text-narrow1">암호를 변경하시려면 아래 버튼을 클릭하세요</span>
							<input type="password" class="updatePass myinfo_input" value="****">
                        </li>
                    </ul>
                </div>
            </div>			
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.idDetail .area04 .storeBuy').on("focus",function () {
                $('.popup.i05').addClass('on');
            });
            ...
            $('.idDetail .area04 .storeAddr').on("focus",function () {
                $('.popup.i07').addClass('on');
            });
			...
			$('.idDetail .area04 .updatePass').click(function () {
                $('.popup.i08').addClass('on');
            });
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.result==true){					
					...
					Swal.fire({html:'계정이 추가되었습니다',timer:1000});
				}
				else Swal.fire(data.message);       
			}
		});		
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.result==true){				
					...
					Swal.fire({html:'이체 되었습니다'}).then(()=>{location.reload();});   
				}
				else Swal.fire(data.message);       
			}
		});		
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.result==true){					
					...
					Swal.fire({html:'변경되었습니다',timer:1000});   
				}
				else Swal.fire(data.message);       
			}
		});		
